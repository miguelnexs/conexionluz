...
import { Award, Printer, Sprout, ShieldCheck, Sparkles } from 'lucide-react';

export default function DPCertificate({ studentName = 'Estudiante de Conexión Luz' }: { studentName?: string }) {
  ...
  const certificateCode = `CL-DP-${Math.floor(100000 + Math.random() * 900000)}`;
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      >
        ...
        <div className="absolute -bottom-12 -left-12 h-40 w-40 bg-teal-50 rounded-full blur-2xl pointer-events-none" />

        {/* Header */}
        <div className="space-y-2">
          <div className="flex items-center justify-center gap-2 text-emerald-800 font-black tracking-widest text-xs uppercase">
            <Sprout className="h-4 w-4" /> PLATAFORMA EDUCATIVA CONEXIÓN LUZ <Sprout className="h-4 w-4" />
          </div>
        ...
        <div className="py-2 border-b-2 border-slate-200 inline-block px-10 min-w-[320px]">
          <h2 className="text-2xl sm:text-4xl font-black text-emerald-950 font-serif italic">
            {studentName}
          </h2>
        </div>
        ...
        <div className="max-w-2xl mx-auto space-y-3">
          <p className="text-xs sm:text-sm text-slate-700 leading-relaxed font-medium">
            Por haber completado satisfactoriamente los 10 módulos académicos, evaluaciones pedagógicas, laboratorio de crecimiento, retos de constancia y diseño de su plan maestro en el programa de formación continua:
          </p>
          <div className="p-4 bg-emerald-50/80 border border-emerald-200 rounded-2xl">
            <h3 className="text-lg sm:text-2xl font-black text-emerald-950 uppercase tracking-tight">
              DESARROLLO PERSONAL
            </h3>
            <p className="text-xs text-emerald-800 font-medium mt-1">
              "Un camino de autoconocimiento, crecimiento y transformación consciente"
            </p>
          </div>
        </div>
        ...
        <div className="pt-6 border-t border-slate-100 grid grid-cols-1 sm:grid-cols-3 gap-6 items-center text-xs">
          ...
          <div className="flex flex-col items-center justify-center space-y-1">
            <div className="h-14 w-14 rounded-full bg-emerald-100 text-emerald-800 border-2 border-emerald-300 flex items-center justify-center font-black shadow-inner">
              <Award className="h-7 w-7" />
            </div>
            <span className="text-[9px] font-black uppercase text-emerald-800 tracking-wider">80 HORAS FORMATIVAS</span>
          </div>
        ...
        <div className="pt-4 border-t border-slate-100">
          <p className="text-[10px] text-slate-600 leading-normal max-w-xl mx-auto">
            Este certificado acredita la finalización de un programa de formación educativa y crecimiento personal en Conexión Luz. No constituye título universitario oficial, certificación clínica ni licencia profesional para el ejercicio de la psicología reglada.
          </p>
        </div>
      </div>
    </div>
