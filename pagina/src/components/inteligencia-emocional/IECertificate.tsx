...
export default function IECertificate({ studentName = 'Estudiante de Conexión Luz' }: { studentName?: string }) {
  ...
  const certificateCode = `CL-IE-${Math.floor(100000 + Math.random() * 900000)}`;
      ...
      >
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-center gap-2 text-teal-800 font-black tracking-widest text-xs uppercase">
            <Heart className="h-4 w-4" /> PLATAFORMA EDUCATIVA CONEXIÓN LUZ <Heart className="h-4 w-4" />
          </div>
        ...
        <div className="py-2 border-b-2 border-slate-200 inline-block px-10 min-w-[320px]">
          <h2 className="text-2xl sm:text-4xl font-black text-teal-950 font-serif italic">
            {studentName}
          </h2>
        </div>
        ...
        <div className="max-w-2xl mx-auto space-y-3">
          <p className="text-xs sm:text-sm text-slate-700 leading-relaxed font-medium">
            Por haber completado satisfactoriamente los 10 módulos formativos, evaluaciones teóricas, laboratorio de autorregulación afectiva, retos de escucha activa y diseño del plan personal en:
          </p>
        ...
        <div className="pt-6 border-t border-slate-100 grid grid-cols-1 sm:grid-cols-3 gap-6 items-center text-xs">
          ...
          <div className="flex flex-col items-center justify-center space-y-1">
            ...
            <span className="text-[9px] font-black uppercase text-teal-800 tracking-wider">80 HORAS FORMATIVAS</span>
          </div>
        ...
        <div className="pt-4 border-t border-slate-100">
          <p className="text-[10px] text-slate-600 leading-normal max-w-xl mx-auto">
            Este certificado acredita la finalización de un programa de formación educativa continua en Conexión Luz. No constituye título universitario oficial, certificación clínica ni licencia profesional para el ejercicio de la psicología sanitaria regulada.
          </p>
        </div>
      </div>
    </div>
