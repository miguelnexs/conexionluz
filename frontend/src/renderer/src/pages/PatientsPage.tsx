...
function userTypeBadgeClass(userType?: string): string {
  const ut = (userType || '').toLowerCase()
  if (ut === 'terapeuta') return 'bg-primary/10 text-primary border border-primary/20'
  if (ut === 'paciente') return 'bg-blue-500/15 text-blue-700 dark:text-blue-300'
...
export function PatientsPage(): JSX.Element {
  ...
  const [togglingIntakeId, setTogglingIntakeId] = useState<number | null>(null)
  ...
  const toggleIntake = async (patient: Patient) => {
    setTogglingIntakeId(patient.id)
    const res = await api.patch<Patient>(`/api/patients/${patient.id}/`, {
      intakeCompleted: !patient.intakeCompleted
    })
    if (res.ok) {
      setItems((prev) =>
        prev.map((p) => (p.id === patient.id ? { ...p, intakeCompleted: !p.intakeCompleted } : p))
      )
    } else {
      setError(res.error)
    }
    setTogglingIntakeId(null)
  }
          ...
          >
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="flex flex-col items-end gap-1.5">
                ...
                <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold ${userTypeBadgeClass(p.userType)}`}>
                  {resolveUserTypeLabel(p.userType)}
                </span>
              </div>
            </div>
            ...
            >
              ...
              <div className="flex flex-col items-center text-center">
                ...
                <p className="mb-8 text-sm text-muted-foreground leading-relaxed max-w-sm mx-auto">
                  {t('patients_admin.confirm_delete_desc', 'Esta acción no se puede deshacer. El usuario será eliminado permanentemente de la plataforma.')}
                </p>
